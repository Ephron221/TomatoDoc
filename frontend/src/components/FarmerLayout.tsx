...
const FarmerLayout: React.FC = () => {
  ...
  const menuItems = [
    ...
    { path: '/dashboard/chats', icon: <History className="w-5 h-5" />, label: 'My Detections' },
    { path: '/dashboard/chat/new', icon: <Camera className="w-5 h-5" />, label: 'New Scan' },
    { path: '/dashboard/experts', icon: <Users className="w-5 h-5" />, label: 'Meet Experts' },
    { path: '/dashboard/payment', icon: <CreditCard className="w-5 h-5" />, label: 'My Subscription' },
    { path: '/profile', icon: <User className="w-5 h-5" />, label: 'My Profile' },
  ];
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 w-72 bg-white border-r border-gray-100 z-50 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 ease-in-out flex flex-col`}>
        <div className="p-8 border-b border-gray-50 flex items-center justify-between">
          <Link to="/" className="flex items-center space-x-3">
            ...
            <span className="text-xl font-black text-gray-900 tracking-tight">Tomato<span className="text-secondary">Doc</span></span>
          </Link>
        ...
        <div className="flex-grow overflow-y-auto py-6 px-4 space-y-2">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] px-4 mb-4">Farmer Dashboard</p>
          {menuItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
        ...
        <div className="p-4 space-y-4">
          <div className="bg-gray-900 rounded-[2rem] p-5 relative overflow-hidden group cursor-pointer" onClick={() => navigate('/pricing')}>
             <div className="relative z-10">
               <p className="text-white text-[10px] font-black uppercase tracking-widest mb-1 opacity-70">Unlock Premium</p>
               <p className="text-white font-bold text-xs leading-relaxed mb-4">Get unlimited scans & 24/7 expert support.</p>
               <div className="flex items-center text-secondary text-[10px] font-black uppercase tracking-widest">
                 Upgrade Now <ChevronRight className="ml-1 w-3 h-3 group-hover:translate-x-1 transition-transform" />
               </div>
             </div>
             <div className="absolute -right-4 -bottom-4 opacity-10 transform rotate-12 group-hover:rotate-0 transition-transform">
          ...
          <div className="bg-gray-50 rounded-[2rem] p-4 flex items-center space-x-3 border border-gray-100">
             ...
             <div className="flex-grow min-w-0">
                ...
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider truncate">Farmer Account</p>
             </div>
             <button 
      ...
      <main className="flex-grow flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center justify-between px-6 lg:px-10 z-30 sticky top-0">
          ...
          <div className="hidden md:flex relative group w-96">
            ...
            <input 
              type="text" 
              placeholder="Search diseases, experts, chats..." 
          ...
          <div className="flex items-center space-x-4">
            <div className="hidden sm:flex items-center bg-green-50 rounded-full px-4 py-1.5 text-[10px] font-black text-green-600 uppercase tracking-widest border border-green-100">
        ...
        <div className="flex-grow overflow-y-auto bg-gray-50/50 p-6 lg:p-10 scrollbar-hide">
          <Outlet />
        </div>
      </main>
    </div>
